                ...
                <form action="{{route('ads.update', $ad)}}" method="post" id="form-submit" class=" fill-details bg-white px-4 px-lg-8 ">
                        ...
                        <div class="row  with-border w-50 textfields py-1 d-flex flex-column align-items-center justify-content-center gap-3  ">
                            ...
                            <div class="letterBlocAd textfield-input mb-2 d-flex justify-content-start align-items-center m-0">
                                ...
                                <select id="first_letter" name="first_letter" class="form-select">
                                    <option value="" {{ old('first_letter', $ad->first_letter) == "" ? 'selected' : '' }} >  </option>
                                    @foreach (config('app')['arabic_letters'] as $key => $value )
                                        <option value="{{$value}}" {{ old('first_letter', $ad->first_letter) == $value ? 'selected' : '' }} > {{$value}} - {{ config('app')['english_letters'][$value] }}</option>
                                    @endforeach  
                                </select>
                                <x-front.form.error field="first_letter" />
                                
                                <select id="second_letter" name="second_letter" class="form-select">
                                    <option value="" {{ old('second_letter', $ad->second_letter) == "" ? 'selected' : '' }} >  </option>
                                    @foreach (config('app')['arabic_letters'] as $key => $value )
                                        <option value="{{$value}}" {{ old('second_letter', $ad->second_letter) == $value ? 'selected' : '' }} > {{$value}} - {{ config('app')['english_letters'][$value] }}</option>
                                    @endforeach 
                                </select>
                                <x-front.form.error field="second_letter" />
                                
                                <select id="third_letter" name="third_letter" class="form-select">
                                    <option value="" {{ old('third_letter', $ad->third_letter) == "" ? 'selected' : '' }} >  </option>
                                    @foreach (config('app')['arabic_letters'] as $key => $value )
                                        <option value="{{$value}}" {{ old('third_letter', $ad->third_letter) == $value ? 'selected' : '' }} > {{$value}} - {{ config('app')['english_letters'][$value] }}</option>
                                    @endforeach 
                                </select>
                        ...
                        <div class="row w-50 with-border textfields py-1 d-flex flex-column align-items-center justify-content-center gap-3  ">
                            ...
                            <div class="numberBlocAd textfield-input mb-2 d-flex justify-content-start align-items-center">
                                ...
                                <select id="first_number" name="first_number"  class="form-select cnt_firstNumber">
                                    <option class="font-tajawal" value="" {{ old('first_number', $ad->first_number) == "" ? 'selected' : '' }} >  </option>
                                    @foreach (config('app')['arabic_numbers'] as $key => $value )
                                        <option class="font-tajawal" value="{{$value}}" {{ old('first_number', $ad->first_number) == $value ? 'selected' : '' }} > {{$value}}</option>
                                    @endforeach
                                </select>
                                ...
                                <select id="second_number" name="second_number" class="form-select cnt_secondNumber mx-2 ms-md-4 me-md-0 ms-lg-4 me-lg-0">
                                    <option class="font-tajawal" value="" {{ old('second_number', $ad->second_number) == "" ? 'selected' : '' }} >  </option>
                                    @foreach (config('app')['arabic_numbers'] as $key => $value )
                                        <option class="font-tajawal" value="{{$value}}" {{ old('second_number', $ad->second_number) == $value ? 'selected' : '' }} > {{$value}}</option>
                                    @endforeach
                                </select>
                                ...
                                <select id="third_number" name="third_number" class="form-select cnt_thirdNumber me-2 me-md-0 me-lg-0">
                                    <option class="font-tajawal" value="" {{ old('third_number', $ad->third_number) == "" ? 'selected' : '' }} >  </option>
                                    @foreach (config('app')['arabic_numbers'] as $key => $value )
                                        <option class="font-tajawal" value="{{$value}}" {{ old('third_number', $ad->third_number) == $value ? 'selected' : '' }} > {{$value}}</option>
                                    @endforeach
                                </select>
                                <x-front.form.error field="third_number" />
                                
                                <select id="fourth_number" name="fourth_number" class="form-select cnt_fourthNumber">
                                    <option class="font-tajawal" value="" {{ old('fourth_number', $ad->fourth_number) == "" ? 'selected' : '' }} >  </option>
                                    @foreach (config('app')['arabic_numbers'] as $key => $value )
                                        <option class="font-tajawal" value="{{$value}}" {{ old('fourth_number', $ad->fourth_number) == $value ? 'selected' : '' }} > {{$value}}</option>
                                    @endforeach 
                                </select>
                            ...
                            <div class="row add-lawha-radio no-gap  d-flex justify-content-center align-items-start flex-row position-relative gap-4 mb-3 "
                                ...
                                <div class="col mb-3 ">
                                    <div class="switch-field">
                                        <div class=" form-check">
                                            <input class="form-check-input" name="validity_of_periodic_examination" value="جديد" type="radio" id="validity_of_periodic_examination_new"  checked {{ old('validity_of_periodic_examination',$ad->validity_of_periodic_examination) == "جديد" ? 'checked' : '' }}>
                    ...
                    <div class="d-flex controls d-flex flex-row align-items-center justify-content-start py-2 gap-1">
                        ...
                        <div class="col textfields py-1 d-flex flex-column align-items-start justify-content-start gap-3 px-0 fieldPhoneFlag">
                            <x-front.form.phone name="phone" class="ltr" placeholder="5xxxxxxxx" id="phone" :value="old('phone', $ad->phone)" />
                        ...
                        <div class="col textfields py-1 d-flex flex-column align-items-start justify-content-start gap-3  px-0 "  style="border-left:#cac7c7 1px solid ; padding-left: 5px;">
                            <div class="textfield-input   m-0 d-flex justify-content-center align-items-center">
                                ...
                                <input type="text" class="font-tajawal bg-transparent text-right custom_input price_input {{ old('in_auction', $ad->in_auction) == 'نعم' ? 'bg-gray' : '' }}" name="price"  value="{{old('price', $ad->price)}}" id="price"  placeholder="100 ريال "  {{ old('in_auction', $ad->in_auction) == 'نعم' ? 'disabled' : '' }} >
                            </div>
